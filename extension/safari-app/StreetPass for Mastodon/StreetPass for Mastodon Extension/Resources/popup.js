...
import { getIconState, getRelMeHrefDataStore, getProfiles, getDisplayHref } from './util.js';
...
if (relMeHrefDataStore) {
  ...
  if (values.length) {
    ...
    userCount.className = "absolute top-12 right-12 leading-[1.3] rounded-6       text-11 text-purple py-[0.18em] px-[0.45em] bg-purple-light font-medium";
    ...
    nullState.className = "absolute top-0 right-0 bottom-0 left-0 flex justify-center items-center      text-gray text-13";
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const prevRelMeHrefDate = prevRelMeHrefData ? new Date(prevRelMeHrefData.viewedAt).getDate() : new Date().getDate();
    const previousItemWasDayBefore = prevRelMeHrefDate !== new Date(relMeHrefData.viewedAt).getDate();
    ...
    dateColumn.appendChild(
      document.createTextNode(
        new Intl.DateTimeFormat(void 0, {
          timeStyle: "short"
        }).format(relMeHrefData.viewedAt).toLowerCase().replace(/\s+/g, "")
      )
    );
